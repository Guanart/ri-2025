...
class IRSystem:
    ...
    def index_collection(self, dir_root: str):
        ...
        def gen_docs(root_dir):
            for root, _, files in os.walk(root_dir):              # os.walk recorre el árbol de directorios y subdirectorios
                for fname in files:
                    if fname.endswith(".html"):
                        path = os.path.join(root, fname)
                        with open(path, 'r', encoding='utf8') as f:
                            text = BeautifulSoup(f, 'html.parser').get_text(separator=' ')  # Concatena el texto de los distintos nodos (tags) HTML con espacios en blanco
                        ...
                        '''
                        yield {'docno': docno, 'text': text}    # Uso de yield para generar un iterable (iter) de diccionarios (gen_docs() es una función generadora)

        if os.path.exists(self.index_path):
            self.index = pt.IndexFactory.of(self.index_path+"/data.properties")
        else:
            indexer = pt.IterDictIndexer(self.index_path, meta={'docno': 50})           # Índice iterable de dicts
    ...
    def compute_correlation(self, res1: pd.DataFrame, res2: pd.DataFrame, qid: int, k: int):
        """
