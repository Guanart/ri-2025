                ...
                if isinstance(a, Tag):
                    ...
                    if isinstance(href, str):
                        norm_url = self.normalize_url(
                            href, base_url, self.preserve_query
                        )
            ...
            while not self.should_stop.is_set():
                try:
                    ...
                    if page:
                        ...
                        with self.count_lock:
                            self.site_page_count[domain] = (
                                self.site_page_count.get(domain, 0) + 1
                            )
                        ...
                        with self.stats_lock:
                            ...
                            if (
                                self.total_processed % 10 == 0
                            ):  # Actualizar cada 10 páginas
                                print(
                                    f"\rWorker-{worker_id}: {self.total_processed} páginas, "
